...
function [D1,D2,D3] = createDDWithDBoundary3Dmsk(N1,N2,N3,mask_indx)
% [D,Dp] = createD(N1,N2)
%
...
%[bdr_ind,bdr_indxMSK]=find_mask_bdr(mask_indx,[N1 N2 N3]);
%[~,bdr_indxMSK]=find_mask_bdr(mask_indx,[N1 N2 N3]);
%if exist('bdr1','var')
%    bdr_indxMSK=bdr1;
%end
...
if (N1 > 1)&&(N2>1)&&(N3>1)   
    ...
    if (numel(e)>2)
        ...
        E = speye(N2);
        E2 = speye(N3);
        D1 = kron(E2,kron(E,T));D1=D1(mask_indx,mask_indx);%D1(bdr_indxMSK,:)=0;
    end
    e = ones(N2,1);
    if (numel(e)>2)
        ...
        E = speye(N1);
        E2 = speye(N3);
        D2 = kron(E2,kron(T,E));D2=D2(mask_indx,mask_indx);%D2(bdr_indxMSK,:)=0;
    end
    e = ones(N3,1);
    if (numel(e)>2)
        ...
        E = speye(N1);
        E2 = speye(N2);
        D3 = kron(T,kron(E2,E));D3=D3(mask_indx,mask_indx);%D3(bdr_indxMSK,:)=0;
    end
else
