function svreg_elastic_vol_reg_nonlin_cg_old(subbasename,subbasename_tmp,nit,regul_alpha)
...
vs_mask=load_nii_z([subbasename,'.mask.nii.gz']);
...
v_w=trilinear(double(v_atlas.img),double(vmap.img(:,:,:,2)),double(vmap.img(:,:,:,1)),double(vmap.img(:,:,:,3)));v_atlas.img=[];
...
function [xn,fn,fcall,alpha]=backtrack_linesearch(xc,d,fc,DDfnc,c,gamma,eps,vs,vt,mask_ind,L,bdr_indxMSK)
if DDfnc >= 0,
