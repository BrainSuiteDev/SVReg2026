...
fp=fopen(logfname,'a+');
fprintf(fp,'SVREG Version 15c(build#2225) (refine_ROIs2)  \n');
...
if isempty(strfind(flags,'gui'))
   disp1('ROIRefinement','svreg_volreg',flags);
else
   disp1(sprintf('ROIRef:%s hemi',hemi),'svreg_volreg',flags);
end
%subbasename=subs;
%subbasename=subbasename_tmp;
if exist('hemi','var')
   disp1(sprintf('Refining ROIs in %s hemi',hemi),'refine_ROIs2',flags);
   subs_out=[subbasename_tmp,'.',hemi,'.mid.cortex.svreg.dfs'];
   %subs_out=[subs,'.',hemi,'.mid.cortex.svreg.dfs'];
   %subs=[subs,'.',hemi,'.mid.cortex.reg.dfs'];
   subs=[subbasename_tmp,'.',hemi,'.mid.cortex.reg.dfs'];
end
...
copy_attrib_colors(subbasename_tmp,hemi);
%copy_attrib_colors(subbasename,'right');
%subbasename=subs;
copyfile([subbasename_tmp,'.',hemi,'.mid.cortex.svreg.dfs'],[subbasename,'.',hemi,'.mid.cortex.svreg.dfs'],'f');
copyfile([subbasename_tmp,'.',hemi,'.inner.cortex.svreg.dfs'],[subbasename,'.',hemi,'.inner.cortex.svreg.dfs'],'f');
copyfile([subbasename_tmp,'.',hemi,'.pial.cortex.svreg.dfs'],[subbasename,'.',hemi,'.pial.cortex.svreg.dfs'],'f');
% copyfile([subbasename_tmp,'.',hemi,'.mid.cortex.svreg.dfs'],[subbasename,'.',hemi,'.mid.cortex.svreg.dfs'],'f');
% copyfile([subbasename_tmp,'.',hemi,'.inner.cortex.svreg.dfs'],[subbasename,'.',hemi,'.inner.cortex.svreg.dfs'],'f');
% copyfile([subbasename_tmp,'.',hemi,'.pial.cortex.svreg.dfs'],[subbasename,'.',hemi,'.pial.cortex.svreg.dfs'],'f');
 
copyfile([subbasename_tmp,'.target.',hemi,'.mid.cortex.reg.dfs'],fullfile(pth,['atlas.',hemi,'.mid.cortex.svreg.dfs']),'f');
%copyfile([subbasename_tmp,'.target.right.mid.cortex.reg.dfs'],fullfile(pth,'atlas.right.mid.cortex.svreg.dfs'),'f');
