...
function refine_vol_labels(subbasename)

%gunzip([subbasename,'.svreg.label.nii.gz']);
vl=load_nii_z([subbasename,'.svreg.label.nii']);
...
sin=readdfs([subbasename,'.left.inner.cortex.svreg.dfs']);
...
sinr=readdfs([subbasename,'.right.inner.cortex.svreg.dfs']);
smidr=readdfs([subbasename,'.right.mid.cortex.svreg.dfs']);
spialr=readdfs([subbasename,'.right.pial.cortex.svreg.dfs']);
spialr2.vertices=0.5*(spialr.vertices+smidr.vertices);
indknr=(smidr.labels~=0);
vl_new=vl;
ind=find((vl_new.img>=100)&(vl_new.img<600)&(vl_new.img~=344)&(vl_new.img~=345)&(vl_new.img~=346)&(vl_new.img~=347));
...
save_untouch_nii_gz(vl_new,[subbasename,'.svreg.ref.label.nii']);
