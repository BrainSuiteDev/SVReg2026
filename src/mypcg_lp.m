...
function [x] = mypcg_lp(A,b,Tol,Maxit,M,x,flags)
if ~exist('flags','var')
    flags='';
end
%  flags=strrep(flags,'-','');
...
for kk=1:NITER
    for i=1:Maxit
        ...
        Ap=double(single(A*p));
        alpha=double(single(rtz/(p'*Ap)));
        x=double(single(x+alpha.*p));
        r=double(single(r-alpha.*Ap));
        z=double(single(Minv.*r));
        newrtz=double(single(r'*z));
        beta=double(single(newrtz/rtz));
        rtz=double(single(newrtz));
        p=double(single(z+beta.*p));
        rtz1(i)=double(single(rtz));
    end
    if rtz<Tol % no need to take abs(rtz) for this preconditioner
        break;
    end
