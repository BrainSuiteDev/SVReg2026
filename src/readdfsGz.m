...
function dfs = readdfsGz(fileName, workdir)
% Reads a BrainSuite Surface file saved as either .dfs or .dfs.gz
% Wrapper around readdfs to support gzipped files. See below for readdfs.
% Usage: 
%     dfs = readdfsGz(fname)
%     dfs = readdfsGz(fname, workdir); % workdir is name of a temporary directory
%
% Added by Chitresh Bhushan (cbhushan@usc.edu)

loc = strfind(fileName, '.gz');
len = length(fileName);
...
if ~isempty(loc) && (len-loc(end)==2)  % .gz extension found
   ...
   dfsfile = gunzip(fileName, workdir);
   [dfs, hdr] = readdfs(char(dfsfile));
   delete(char(dfsfile));
   
   if temp_folder % delete workdir if created
      rmdir(workdir, 's')
   end
else
   [dfs, hdr] = readdfs(fileName);
end
