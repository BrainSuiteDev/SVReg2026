...
if [ -z "$BrainSuiteMCR" ]; then
  if [ -e /Applications/MATLAB/MATLAB_Runtime/R2025b ]; then
    ...
    echo
    echo "Could not find Matlab 2025b with Matlab Compiler or Matlab 2025b (v25.2)."
...
read -d '' usage <<EOF
  ...
  svreg_thickness2atlas : Usage: svreg_thickness2atlas.sh subbasename
...
$SVREG_EXEC "${exe_dir}"/svreg_thickness2atlas.app/Contents/MacOS/svreg_thickness2atlas "${FILEPREFIX}"
